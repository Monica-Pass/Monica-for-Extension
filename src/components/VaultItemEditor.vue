...
function initialize() {
  ...
  switch (props.item.kind) {
    ...
    case "totp": Object.assign(fields, { secret: props.item.secret, issuer: props.item.issuer || "", accountName: props.item.accountName || "", algorithm: props.item.algorithm, digits: String(props.item.digits), period: String(props.item.period) }); break;
  }
}
...
function submit() {
  ...
  if (kind.value === "totp" && !fields.secret.replace(/\s+/g, "")) return void (error.value = "请输入 TOTP 密钥。");
...
function buildItem(title: string): VaultItem {
  ...
  switch (kind.value) {
    ...
    case "totp": return { ...base, kind: "totp", secret: fields.secret.replace(/\s+/g, "").toUpperCase(), issuer: optional(fields.issuer), accountName: optional(fields.accountName), algorithm: fields.algorithm, digits: clampNumber(fields.digits, 6, 6, 10), period: clampNumber(fields.period, 30, 5, 300) } satisfies TotpItem;
  }
}
...
function emptyFields() {
  return {
    ...
    content: "", secret: "", issuer: "", algorithm: "SHA1" as TotpItem["algorithm"], digits: "6", period: "30"
  };
}
</script>
  ...
  <div class="modal-backdrop" role="presentation" @mousedown.self="emit('cancel')">
    <section class="editor-dialog vault-item-dialog" role="dialog" aria-modal="true" aria-labelledby="vault-item-editor-title">
      ...
      <form class="editor-form vault-item-form" @submit.prevent="submit">
        ...
        <template v-else><label class="field field-wide"><span>TOTP 密钥 *</span><input v-model="fields.secret" type="password" autocomplete="off" /><small>支持 Base32 密钥；空格会在保存时移除。</small></label><label class="field"><span>签发方</span><input v-model="fields.issuer" /></label><label class="field"><span>账户</span><input v-model="fields.accountName" /></label><label class="field"><span>算法</span><select v-model="fields.algorithm"><option>SHA1</option><option>SHA256</option><option>SHA512</option></select></label><label class="field"><span>位数</span><input v-model="fields.digits" type="number" min="6" max="10" /></label><label class="field"><span>周期（秒）</span><input v-model="fields.period" type="number" min="5" max="300" /></label></template>
