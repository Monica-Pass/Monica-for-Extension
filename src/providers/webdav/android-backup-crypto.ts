...
export async function decryptAndroidBackup(bytes: Uint8Array, password: string): Promise<Uint8Array> {
  if (!isAndroidEncryptedBackup(bytes)) return bytes;
  ...
  try {
    const key = await deriveAndroidBackupKey(password, salt);
    return new Uint8Array(await crypto.subtle.decrypt({ name: "AES-GCM", iv: iv as BufferSource, tagLength: 128 }, key, ciphertext));
  } catch {
