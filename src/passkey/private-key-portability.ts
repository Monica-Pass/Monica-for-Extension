...
export function parsePortablePasskeyPrivateKey(value: unknown): PortablePasskeyPrivateKey | undefined {
  ...
  const oid = pkcs8AlgorithmOid(padded);
  const algorithm = oid === "1.2.840.10045.2.1" ? -7 : oid === "1.2.840.113549.1.1.1" ? -257 : oid === "1.3.101.112" ? -8 : undefined;
  return algorithm ? { pkcs8Base64: padded, algorithm } : undefined;
}

function pkcs8AlgorithmOid(value: string): string | undefined {
