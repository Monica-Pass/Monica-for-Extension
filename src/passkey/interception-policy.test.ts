...
describe("Passkey interception policy", () => {
  it("keeps unsupported registration requests with the browser", () => {
    ...
    expect(shouldInterceptPasskeyCreate({ ...base, userVerification: "required" })).toBe(false);
  ...
  it("keeps conditional, silent, UV-required and external authentication native", () => {
    ...
    expect(shouldInterceptPasskeyGet({ ...base, userVerification: "required" })).toBe(false);
